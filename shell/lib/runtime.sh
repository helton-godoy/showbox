...
showbox_set() {
    [[ $# == 3 ]] || { printf 'Uso: showbox_set widget propriedade valor\n' >&2; return 64; }
...
_sb_run() {
    ...
    trap '' PIPE
    command -v setsid >/dev/null || { printf 'Showbox: instale util-linux (setsid).\n' >&2; return 127; }
    command -v "${SHOWBOX_BIN:-showbox}" >/dev/null || { printf 'Showbox: executável não encontrado; configure SHOWBOX_BIN.\n' >&2; return 127; }
